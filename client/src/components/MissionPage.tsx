...
const MissionPage: React.FC<MissionPageProps> = ({ onBack }) => {
  ...
  const [categoryLabel, setCategoryLabel] = useState('Missions')
  const [activeCard, setActiveCard] = useState<MissionCardType | null>(null)
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        if (cancelled) return
        setQuote(data.quote || '')
        setCategoryLabel(data.category?.label ?? 'Missions')
