...
const GalleryPage: React.FC<GalleryPageProps> = ({
    ...
    useEffect(() => {
        const load = async () => {
            ...
            setLoading(true)
            try {
                const [categoriesRes, imagesRes] =
                    await Promise.all([
                        fetch("/api/gallery/categories"),
                        fetch("/api/gallery/images")
                    ])
                if (!categoriesRes.ok) {
                    const body = await categoriesRes
                        .json()
                        .catch(() => ({}))
                    throw new Error(
                        body?.error ||
                            `Failed to load gallery categories: ${categoriesRes.status}`
                    )
                }
                if (!imagesRes.ok) {
                    const body = await imagesRes
                        .json()
                        .catch(() => ({}))
                    throw new Error(
                        body?.error ||
                            `Failed to load gallery images: ${imagesRes.status}`
                    )
                }
                const categoryData =
                    (await categoriesRes.json()) as GalleryCategory[]
                const imageData =
                    (await imagesRes.json()) as GalleryImage[]
        ...
        >
            ...
            <div className="p-4 sm:p-6 spiritual-inset">
                {loading ? (
                    ...
                    <div className="rounded-xl border border-memorial-line bg-memorial-card spiritual-card-depth p-8 sm:p-10 text-center">
                        <p className="text-red-600 text-sm leading-relaxed max-w-sm mx-auto">
                            {error}
                        </p>
                    </div>
                ) : categories.length === 0 ? (
                    <div className="rounded-xl border border-dashed border-memorial-line bg-memorial-card spiritual-card-depth p-8 sm:p-10 text-center">
                        <ImageIcon
                            className="w-12 h-12 text-memorial-accent/50 mx-auto mb-4"
                            strokeWidth={0.9}
                        />
                        <p className="text-memorial-muted text-base leading-relaxed max-w-sm mx-auto">
                            No gallery categories have been
                            created yet.
                        </p>
                    </div>
